...
class HTMLConfigSyncer {
  ...
  extractSelectors(document) {
    ...
    // Username input
    const usernameInput = document.querySelector('input[name="username"], input[id="username"], input[type="text"]');
    if (usernameInput) {
      selectors.username_input = this.generateSelector(usernameInput);
    }

    // Password input
    const passwordInput = document.querySelector('input[name="password"], input[id="password"], input[type="password"]');
    if (passwordInput) {
    ...
    // Welcome message
    const welcomeDiv = document.querySelector('#welcome, .welcome, [data-testid="welcome"]');
    if (welcomeDiv) {
      selectors.welcome_message = this.generateSelector(welcomeDiv);
    }

    // Error message
    const errorDiv = document.querySelector('.error-message, .error, [data-testid="error"]');
    if (errorDiv) {
  ...
  generateChangeReport(changes) {
    const reportPath = path.join(__dirname, '..', 'reports', 'config-changes.md');
